...
export class AllJobsComponent implements OnInit, OnDestroy {
  ...
  get_filter_agent_process() {
    this.jobService.getAgentName().subscribe((data: any) => {
  ...
  filter_parmas_jobId(job_id) {
    ...
    this.jobService
      .JobsFilter(`id+eq+guid'${job_id}'`, this.page.pageSize, skip)
      .subscribe((data: any) => {
  ...
  gotodetail(id) {
    this.router.navigate([`/pages/job/view/${id}`]);
  }
  ...
  filter_job() {
    ...
    if (this.filter_agent_id != null && this.filter_agent_id != '') {
      this.abc_filter =
        this.abc_filter + `agentID+eq+guid'${this.filter_agent_id}' and `;
    }
    if (this.filter_process_id != null && this.filter_process_id != '') {
      this.abc_filter =
        this.abc_filter +
        `${automationsApiUrl.automationId}+eq+guid'${this.filter_process_id}' and `;
    }
    if (this.filter_jobstatus != null && this.filter_jobstatus != '') {
      this.abc_filter =
        this.abc_filter + `jobstatus+eq+'${this.filter_jobstatus}' and `;
    }
    if (this.filter_successful != null && this.filter_successful != '') {
      this.abc_filter =
        this.abc_filter + `isSuccessful+eq+${this.filter_successful} and `;
    }
  ...
  per_page(val) {
    if (this.abc_filter) {
      ...
      const skip = (this.page.pageNumber - 1) * this.page.pageSize;
      this.jobService
        .getAllJobs(this.page.pageSize, skip)
        .subscribe((data: any) => {
          for (let ab of data.items) {
            for (let status of this.jobStatus) {
              if (ab.jobStatus == status.id) {
  ...
  get_AllJobs(top, skip) {
    ...
    this.jobService.getAllJobs(top, skip).subscribe((data: any) => {
      for (let ab of data.items) {
        for (let status of this.jobStatus) {
          if (ab.jobStatus == status.id) {
            ab.jobStatus = status.name;
  ...
  exportFile() {
    if (this.abc_filter) {
      ...
      this.abc_filter == '' ||
      this.abc_filter == undefined ||
      this.abc_filter == null
    ) {
      let fileName: string;
      this.jobService.getExportFile().subscribe((data: HttpResponse<Blob>) => {
        fileName = data.headers
          .get('content-disposition')
          .split(';')[1]
          .split('=')[1]
